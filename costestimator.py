...
def calculate_compute_cost(edition_type, num_departments, warehouse_size_dl, no_of_days_dl, no_of_hours_per_day_dl,
                           department_data):
    ...
    if warehouse_size_dl == 'x-small':
        ...
        raise ValueError(
            "Invalid warehouse size for Data Loading Requirement. Please choose 'x-small', 'small', 'medium', 'large', 'x-large', or '2x-large'")
    ...
    for _ in range(num_departments):
        ...
        if warehouse_size_cr == 'x-small':
            ...
            raise ValueError(
                "Invalid warehouse size. Please choose 'x-small', 'small', 'medium', 'large', 'x-large', or '2x-large'")
        ...
        department_data.append({'Department Name': dept_name, 'Credits consumed Individually': dept_compute_req,
                                'Individual Compute Cost': dept_compute_req_edt})
...
def generate_cost_estimate():
    ...
    compute_cost_tb, data_loading_req, data_loading_req_edt = calculate_compute_cost(edition_type, num_departments,
                                                                                     warehouse_size_dl, no_of_days_dl,
                                                                                     no_of_hours_per_day_dl,
                                                                                     department_data)
